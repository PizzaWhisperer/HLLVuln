...
\subsection{Attacking Redis implementation}
For completeness, we attack the HLL Beta implementation used in the widespread Redis framework~\cite{redis}, which we recall is different than the original HLL algorithm given in~\cite{hll}.
...
Furthermore, the number of buckets is constant and set to 16,384. Those informations allow to gain the knowledge of an S2 adversary.
As a classic Redis application separates the server, owning the sketch, from the client, interacting with the sketch, we consider that the scenarios S3 or S4, while possible, are not the typical Redis use case. 
Thus, we only consider a limited adversary, and implement our attacks under the scenario S2 as described in Section~\ref{sec:redis}.
For simplicity, we set $\epsilon$ to $\frac{1}{2}$ in the attack on an initially empty HLL sketch.
The results are averaged over 30 iterations and reported in Table~\ref{table:tab3}.
Similarly to the section above, our empirical results confirm our theoretical analysis.
