...
\author{}
\else
\author{Kenneth G. Paterson\\ETH Zurich\\\texttt{kenny.paterson@inf.ethz.ch} \and Mathilde Raynal\\EPFL\\\texttt{mraynal@student.ethz.ch}}
\fi % end anon processing of authors 
...
Furthermore, the number of buckets is constant and set to 16,384. Knowledge of theese parameters places us in the setting of an S2 adversary.
Since a classic Redis application separates the server owning the sketch from the client interacting with the sketch, we consider that the scenarios S3 or S4, while possible, are not typical Redis use case. 
...
$\mbox{Adv}^{\mbox{\tiny S$i$-RoI}}_{\mbox{\tiny{HLL-$F$, ${\mathcal{A}}$, ${\mathcal{S}}$}}}({\mathcal{E}}) \leq \epsilon$. 
\end{definition}

In the above definition, we quantify over \texttt{insert($\cdot$)} queries (via $q$) and not other types of query because only \texttt{insert($\cdot$)} queries will appear in our concrete security analysis and we do not want to clutter the notation.

Note that our security analysis using a simulation-based definition does not require us to specify a winning condition for adversary $\mathcal{A}$. Rather, we infer from inspecting the simulator what the adversary can (or cannot) achieve in the ideal world, and show that no efficient algorithm $\mathcal{E}$ can distinguish the real and ideal worlds. This is in contrast to the game-based approach of~\cite{CCS:ClaPatShrPS19} which needs a complex definition to say when the adversary has successfully manipulated a probabilistic data structure. This reveals a strength of the simulation-based approach in more complex settings like ours.

\vspace{0.1cm}
\noindent\textbf{A specific simulator ${\mathcal{S}}$.}
We next describe the simulator ${\mathcal{S}}$ that we will use in our further analysis: ${\mathcal{S}}$ operates by running ${\mathcal{A}}$ in a blackbox manner and handling all of its queries as follows: 
\begin{itemize}
...
For our definition to meaningfully capture security against cardinality manipulation, it is important to restrict ${\mathcal{S}}$ to doing random sampling when handling \texttt{insert($\cdot$)} queries. Otherwise, our definition could be vacuously satisfied (for example ${\mathcal{S}}$ could just sample the PRF key $K$ for itself and provide a perfect real-world view). This is somewhat different to common simulation-based security definitions, where ${\mathcal{S}}$'s input might be restricted (e.g.\ in a confidentiality definition, ${\mathcal{S}}$ might receive only $|m|$ in place of $m$) but its behaviour is not. Crucially, in our setting, security arises from inspecting the details of ${\mathcal{S}}$ and observing that it instantiates HLL in the average case.

We require one further standard definition.
\begin{definition}
Let $F: \{0,1\}^k \times {{\mathcal{D}}} \rightarrow  \{0,1\}^{n+\ell}$.
...
The authors thank Anupama Unnikrishnan for useful discussions on simulation-based definitions and Tom Shrimpton for suggesting to look at GoogleBigQuery.
\fi
