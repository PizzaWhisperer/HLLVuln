...
\vspace{0.5em}
\textbf{Program} HyperLogLog (\textbf{input} $\mathcal{M}$ : multiset of items from domain $\mathcal{D}$).\\
...
\textbf{for} $v \in \mathcal{M}$ \textbf{do}\\
    ...
    \hspace{2em} \textbf{set} $j = 1 + <x_1x_2...x_{b}>_2$; \hfill \textit{\{the binary address determined by the first $b$ bits of $x$\}}\\
    \hspace{2em} \textbf{set} $w := x_{b+1}x_{b+2}...$;\\
