...
We stress that none of our attacks are particularly deep in cryptanalytic terms. Fundamentally, they arise from implementations of HLL using a fixed hash function $h$ internally. Yet HLL is being increasingly widely deployed, and never with protections against our attacks (insofar as we are aware). So despite their technical simplicity, we argue that our attacks are important in establishing the limitations of HLL in the broadest set of use cases. We present our attacks in an abstract way rather than tying them to any specific setting in order to illustrate their generality. We do give a more detailed exploration of their impact on the data-centre network monitoring system proposed in~\cite{flexswitch}, see Section~\ref{sec:exp}. 
...
\subsection{Potential Harms and Mitigations}\label{sec-harms}

Our attacks are presented in a general way and evaluated either abstractly or in the context of a specific system described in the scientific literature~\cite{flexswitch}. We also propose an easily deployable countermeasure. Given this, we think the potential for harm to systems and individuals relying on HLL today is small. On the other hand, we hope our results will spur practitioners to consider the possibility of adversarial inputs in their specific applications and design accordingly. For the specific case of Redis, a real system that we analyse in detail, we have contacted the Redis core team and shared our results, following their advertised security policy.\footnote{\url{https://github.com/redis/redis/security/policy}.}

\subsection{Paper Organisation}
The rest of the paper is organised as follows. Section~\ref{sec:related} immediately below discusses related work. Section~\ref{sec:overview} gives an overview of HLL. Section~\ref{sec:attacks} informally presents our different adversarial models. In Section~\ref{sec:manip}, we describe attacks on HLL and evaluate their impacts under the different adversarial models. Section~\ref{sec:redis} considers the security of the more modern version of HLL due to Ertl~\cite{hllnew} that is used in Redis. Our experimental results are discussed and related back to a specific application setting in Section~\ref{sec:exp}. Section~\ref{sec:formal} provides our formal security model and security proof for HLL using a VIL-PRF. Section~\ref{sec:conclusions} provides our conclusions and ideas for future work.
...
Recent papers~\cite{Ben-EliezerJWY20,DBLP:journals/corr/abs-2011-07471,HassidimKMMS20,KaplanMNS21} have studied broad classes streaming algorithms under adversarial input from a theoretical viewpoint. Our applied perspective complements this work, focussing on the real-world usage of a specific cardinality estimator. 
...
\item[S2:] The adversary has access to the details of the HLL implementation, i.e.\ it knows the number of buckets $m=2^n$ and the hash function $h$ in use. It can insert items into the HLL sketch, but is otherwise blind. In particular, an S2 adversary does not have access to the values of the HLL cardinality estimate at any point in its attack. This kind of attack scenario is appropriate in the context of the port scanning attack of~\cite{portscanhll}, where the adversary has no API to the target HLL sketch but can cause entries to be inserted into it by a network monitor. 
\item[S3:] The adversary has access to the details of the HLL implementation, i.e.\ it knows the number of buckets $m=2^n$ and the hash function $h$ in use. Additionally, it can access and interact with the sketch via an API provided by the sketch owner, allowing it to insert items into the HLL sketch and ask for the HLL's cardinality estimate at any point in time. However, it does not know the individual bucket contents. This scenario corresponds to a client-server setting with the adversary playing the role of the client.
\item[S4:] The adversary has direct access to the HLL sketch and all its internals, i.e., the number of buckets, the hash function $h$, and also the contents of each bucket in the HLL sketch \emph{at one specific point in time}, i.e.\ it is given a \emph{snapshot} of the sketch. It can access and interact with the sketch via an API provided by the sketch owner, as per scenario S3. This scenario models settings where the service hosting the HLL sketch has been compromised by the adversary (and where the adversary continues to act as a client, as in S3).
...
The S2 adversary can insert items and knows the hash function $h$ and number of buckets $m$, but is otherwise blind. Such an adversary is quite realistic. For example, both the Redis and Presto implementations of HLL use a fixed hash function and parameters that can be read off from source code.\footnote{See \url{https://github.com/redis/redis/blob/unstable/src/hyperloglog.c\#L397}, \url{
https://github.com/redis/redis/blob/unstable/src/hyperloglog.c\#L467} for Redis and 
\url{https://github.com/airlift/airlift/blob/master/stats/src/main/java/io/airlift/stats/cardinality/HyperLogLog.java\#L58}
for Presto.}
...
As a second example, setting $B = 1$, we can expect to insert $|{\mathcal{X}}|/m$ items with a final cardinality estimate equal to $m \cdot \ln (1+\frac{1}{m-1})$, which is rounded to $1$ by the HLL algorithm (for $m$ of practical interest, e.g.\ $m=256$). Thus we keep the HLL cardinality estimate to a constant (1!) whilst being able to insert a constant fraction $1/m$ of the items from the input set ${\mathcal{X}}$.
...
\paragraph{\textbf{Complex Attack}}
...
\paragraph{\textbf{Analysis of the Complex Attack}}
...
We assume that at least $2^{t-1} m\ln m$ items have already been honestly inserted into the target HLL, for some integer $t$ with $1 \leq t \leq \ell$ which will be a parameter of our attack. We assume the S2 adversary somehow knows that this condition holds (e.g.\ through knowledge of when the HLL was initialised and the average rate at which items are inserted into it). If it were to be the case that our assumption does not hold and the adversary cannot estimate the number of items in the sketch, they can use a downward estimate such as $t=1$. As this lower-bound on $t$ is highly likely to happen, the adversary is taking only very small risk. Notice that an S3 adversary can use its ability to get cardinality estimates to obtain this knowledge; this is one situation where an S3 adversary can make good use of the extra information available to it as compared to an S2 adversary. In S4, the adversary can use its access to a snapshot to mount a more powerful attack; see Section~\ref{sec:nonemptyHLLS4}. 

Under the condition that at least $2^{t-1} m\ln m$ items have already been honestly inserted, it is highly likely that every one of the $m=2^n$ buckets has counter value $M[j]$ with $M[j] \ge t$. This follows from the standard analysis of the coupon collector problem~\cite[Section 3.6]{MR95} under our usual assumption that $h$ behaves as a random function. To see this, note that when an item $x$ is inserted into a specific bucket $j$, the condition $M[j] \ge t$ holds with probability $q:=1/2^{t-1}$; so with a total of $N$ attempted insertions, we expect $Nq$ usable insertions, with a sharp concentration around this number. We need about $m\ln m$ usable insertions to occur to ensure we get one in every bucket; setting $Nq = m\ln m$ completes the argument. 

With $2^{t-1}m \ln m$ honest insertions, it is also highly likely that the small range correction (Hit Counting algorithm) will not be used, and the target HLL's cardinality estimate will be computed using the raw estimate.
%\[
...
The S2 adversary knows $h$ so it selects items $x$ from ${\mathcal{X}}$ for which the $\ell$ rightmost bits of $h(x)$ start with up to $t-1$ ``0'' bits. In this case, since the relevant bucket $j$ already has $M[j] \ge t$ (with high probability), the bucket's counter is not updated, and the cardinality estimate of the HLL sketch remains unchanged. 

%% half of hash values begin 1...; 1/4 begin 01 (t=2), etc....

The S2 adversary then expects to be able to insert a fraction $1-1/2^t$ of the items from ${\mathcal{X}}$ into the HLL sketch, without increasing the HLL cardinality estimate at all. Notice how the fraction increases towards 1 as parameter $t$ increases: the more items are originally in the sketch, the more items the adversary expects to be able to insert without penalty.

If some buckets are actually empty or contain values less than $t$ (but the adversary does not know which ones in the setting of S2), then we may expect an increase in the HLL cardinality estimate in the above attack. If the adversary suspects this is the case, then it can first insert $m$ carefully selected items into the target HLL sketch to ensure that $M[j] \ge t$ for every bucket. This should increase the HLL cardinality estimate to about $\alpha_m 2^t m$. Then it can mount the attack, and be sure that it does not increase the cardinality estimate any further.

\subsubsection{Initially Non-empty HLL Sketch, Scenario S4}\label{sec:nonemptyHLLS4}
The adversary can adopt the same strategy as for S2, but relax the sampling requirements with the additional information available in this scenario (namely, the count values $M[j]$ per bucket in the HLL sketch). Specifically, if $h(x)$ is mapped to the $j$-th bucket containing $M[j] \ge 0$, then the adversary is satisfied when there is a ``1'' bit in any of the first $M[j]$ positions in this substring. (A value $M[j]=0$ then never permits insertions, meaning an empty bucket stays empty.) Under these conditions, the estimates in the buckets are never updated, and there is no effect on the cardinality estimate made by HLL.
...
In scenarios S2 and S4 with an initially non-empty HLL sketch, we assume that $2^{t-1}m \ln m$ items have been honestly inserted, as per our analysis in Section~\ref{sec:nonempty}. This ensures that, with high probability, we have $C_0 = C_1 = \cdots = C_{t-1} = 0$. In particular, since $C_0 = 0$, $\sigma$ is evaluated at 0 in (\ref{eqn:ertl}), where its value is 0. This means that the estimate $\hat{\lambda}$ is computed using a denominator of the form:
\[
\sum_{k=t}^{\ell}C_k 2^{-k} + m \tau(1-C_{\ell+1}/m)2^{-\ell}.
\]
Now we can proceed exactly as in Section~\ref{sec:nonempty}, inserting items $x$ whose hashes have rightmost $\ell$ bits beginning with up to $t-1$ ``0'' bits (and with a more relaxed condition in S4). With high probability, these insertions do not alter the values of the $C_k$, and hence $\hat{\lambda}$ should be unaffected. In S2, we expect to insert a fraction $1-1/2^t$ of the items in ${\mathcal{X}}$ without increasing the cardinality estimate $\hat{\lambda}$ at all. In S4, the analysis is the same as in Section~\ref{sec:nonemptyHLLS4}.
...
We create an HLL sketch using $n=8$ and hence $m=256$ buckets. We run attacks in each of the scenarios S1, S2 and S4, with initially empty and non-empty HLL sketches. In the latter case, we initialise the sketch with $2^{t-1} m \ln m$ random items, where $t$ is an integer parameter that we vary in our experiments (cf.\ Section~\ref{sec:nonempty}). We then challenge the adversary to pick the largest possible number of items to add to the sketch from our set of  $2^{20}$ possible inputs while trying to keep the cardinality estimate as low as possible. As a second experiment, and in order to get results that can be compared to the work of~\cite{hllvuln}, we mount our attack in their chosen setting for scenario S1: instead of picking from ${\mathcal{X}}$, the adversary has to choose what to insert from a subset of 250,000 random items that are given to it. All attacks were run 50 times and we report average outcomes. 

Our results are reported in Tables~\ref{table:tab1},~\ref{table:tab2} and Figures~\ref{fig:graphS2},~\ref{fig:graphS4}. The tables detail our results for the case of initially empty HLL sketches while the figures show results in S2 and S4 for initially non-empty HLL sketches.


%Finally, for completeness, we also run the attacks against HLL sketches filled with adversarial inputs only, thus that are initially empty. We vary the value of $B$ in the attack for scenario S1 to demonstrate some of the available trade-offs. For simplicity and to keep the attack as striking as possible, we set $B$ to 1 in the attacks on an initially empty HLL sketch in scenarios S2 and S4. All attacks were run 50 times and we report average outcomes. Our results are reported in Tables~\ref{table:tab1} and~\ref{table:tab2}. These tables detail the estimated cardinality at the beginning of the attack, the number of items added by the adversary, and the cardinality estimate after adding the adversary's items.

These experimental results broadly confirm our analyses. First, when attacking an empty HLL sketch (cf.\ Table~\ref{table:tab1}), we can see that in all scenarios, the final cardinality estimate is always exactly a rounded version of the Hit Counting estimator $m \cdot \ln (m/(m-B))$ after inserting a fraction $\frac{B}{m}$ of the items, as predicted by our analysis. For example, in scenario S1 with $B=128$, the final cardinality estimate is 177 (in agreement with the formula) while with $B=1$ in scenarios S2, S4, the final cardinality estimate is just 1 (again, as predicted by the formula). Moreover, the number of items we can insert on average aligns well with our theoretical predictions (e.g.\ $4106$ in S2 and S4, compared with a prediction of $2^{20}/2^8 = 4096$).

Second, Figures~\ref{fig:graphS2},~\ref{fig:graphS4} show very close agreement between the predictions made by our analysis in Section~\ref{sec:nonempty} and our experimental results for the situation where the HLL sketch is initially non-empty. The only significant difference is in scenario S2, where there is a small increase in the cardinality estimate, around 1-2\% on average, as a result of the attack, whereas the theoretical analysis suggests there should be no increase. The reason for this is that the insertion of the initial random $2^{t-1} m\ln m$ items is not guaranteed to produce a situation in which every bucket contains a value $t$ or greater -- this condition holds only with high probability. The result is that some bucket contents are \emph{increased} to $t$ as a result of insertions during the course of the attack, leading to a slightly increased cardinality estimate.

We can also compare with the attack presented in~\cite[Section 5.1]{hllvuln}. In that paper, the authors target the Redis implementation of HLL and perform the attack against an empty sketch filled with adversarial inputs only. Given a set of 250,000 items, the adversary could choose ``74,390 distinct items and obtain an HLL estimate of only 15,780''~\cite{hllvuln}, achieving a five-fold reduction from the true cardinality after 4 iterations (resets). As explained in Section~\ref{sec:redis}, the Redis implementation analysed in~\cite{hllvuln} does not conform precisely to the original HLL algorithm as presented in Figure~\ref{fig:hll}. Also as discussed in Section~\ref{sec:redis}, the Redis implementation has changed its HLL algorithm several times in recent releases, but the authors of~\cite{hllvuln} do not specify which version was used in their experiments. This makes it difficult to make a precise comparison with the results of~\cite{hllvuln}. Nevertheless, the S1 adversary following our strategy against the classical HLL algorithm is able to add 977 and 125,091 distinct items while holding the cardinality estimate at 1 and 177, respectively (see Table~\ref{table:tab2}). Compared to~\cite{hllvuln}, we can reduce the cardinality estimate by a factor of almost 1,000 instead of 5. This comes at the cost of performing significantly more resets compared to~\cite{hllvuln} (more than 1,000 compared to 3). We recall that in practice each reset may cause a delay (for example, it may involve rebooting a network device); we also recall that we consider S1 to be a somewhat artificial attack scenario.

\begin{table*}[htp]
\caption{Attacks on empty sketches for a classic HLL implementation, averaged over 50 iterations.}
\begin{center}
\begin{tabular}{| l | >{\raggedleft\arraybackslash}p{5em} | >{\raggedleft\arraybackslash}p{5em} | >{\raggedleft\arraybackslash}p{5em} | >{\raggedleft\arraybackslash}p{5em} |}
	\hline
	\textbf{Scenario} &  \multicolumn{2}{c|}{S1} & \multicolumn{1}{c|}{S2} & \multicolumn{1}{c|}{S4}  \\ \hline
	\textbf{Original Card. Est.} & 0 & 0 &  0  & 0   \\ \hline
	\textbf{\# Targeted buckets $B$} & 1 & 128 ($m/2$) & 1 &  1 \\ \hline
	\textbf{\# Items added} & 4,132 & 523,791  & 4,106 & 4,106 \\ \hline
	\textbf{Final Card. Est.} & 1 & 177 & 1  & 1 \\ \hline
	\textbf{\# Resets} & 4,718 & 6,973 & - & - \\ \hline
\end{tabular}
\end{center}
\label{table:tab1}
\end{table*}%
...
%\begin{table*}[tb!]
%\small\centering
%\caption{Original Table: Attacks on empty sketches for a classic HLL implementation, averaged over 50 iterations.}
%\medskip
%     \makebox[\linewidth]{
%\begin{tabular}{| m{11em} | m{3.4em} | m{4.8em} | m{4.1em} | m{4.1em} | m{4.1em} | m{4em} |}
%    \hline
%    \textbf{Scenario} &  \multicolumn{2}{c|}{S1} & \multicolumn{2}{c|}{S2} & \multicolumn{2}{c|}{S4} \\ \hline
%    %\textbf{HLL initially empty?} & yes & yes & yes & no & yes & no \\ \hline
%    \textbf{Original Card. Est.} & 0 & 0 &  0 & 1,001 & 0 & 1,007  \\ \hline
%    \textbf{\# Targeted buckets $B$} & 1 & 128 (= m/2) & 1 & - & 1 & - \\ \hline
%      \textbf{\# Items added} & 4,132 & 523,791  & 4,106 & 524,851 & 4,106 & 857,150 \\ \hline
%     \textbf{Final Card. Est.} & 1 & 177 & 1 & 1,025  & 1 &  1,007 \\ \hline
%    \textbf{\# Resets} & 4,718 & 6,973 & - & - & - & - \\ \hline
%
%\end{tabular}}
%\label{table:tab1bis}
...
\begin{table*}[tb!]
\caption{Comparison of attack results for a classic HLL implementation in the S1 adversarial scenario, \\ in the setting of~\cite{hllvuln}.}
\begin{center}
\begin{tabular}{| l | >{\raggedleft\arraybackslash}p{6.8em} | >{\raggedleft\arraybackslash}p{6.8em} | >{\raggedleft\arraybackslash}p{6.8em} |}
    \hline
    \textbf{Attack} & \multicolumn{2}{c|}{Ours} &\multicolumn{1}{c|}{~\cite{hllvuln}} \\ \hline
      \textbf{Original Card. Est.} & 0 & 0 & 0  \\ \hline
       \textbf{\# Targeted buckets $B$} & 1 & 128 ($m/2$) & - \\ \hline
     \textbf{\# Items added} & 977 & 125,091 & 74,390 \\ \hline
    \textbf{Final Card. Est.} & 1 & 177 &  15,780 \\ \hline
    \textbf{\# Resets} & 1,148 & 1,655 & 3 \\ \hline
...
\begin{figure*}[htp]
    ...
    \begin{minipage}{0.45\textwidth}
        \centering
        \caption{Attack results as function of t in scenario S2 with initially non-empty HLL sketch.}\label{fig:graphS2}
        \includegraphics[width=0.9\textwidth]{figures/graph_S2.pdf}
    \end{minipage}\hfill
    \begin{minipage}{0.45\textwidth}
        \centering
        \caption{Attack results as function of t in scenario S4 with initially non-empty HLL sketch.}\label{fig:graphS4}
        \includegraphics[width=0.9\textwidth]{figures/graph_S4.pdf} % second figure itself
     \end{minipage}
     
     \begin{minipage}{0.45\textwidth}
        \centering
        \caption{Attack results as function of t for HLL in Redis (scenario S2) with initially non-empty HLL sketch.}\label{fig:graphRedis}
        \includegraphics[width=0.9\textwidth]{figures/graph_redis.pdf} % second figure itself
     \end{minipage}
\end{figure*}

%\begin{table*}[tb!]
%\small\centering
%\caption{Original Table: Attack results for a classic HLL implementation, averaged over 50 iterations, in the setting of~\cite{hllvuln}.}
%\medskip
%     \makebox[\linewidth]{
%\begin{tabular}{| m{11em} | m{3.4em} | m{4.8em} | m{4.1em} | m{4.1em} | m{4.1em} | m{4em} |}
%    \hline
%    \textbf{Scenario} & \multicolumn{2}{c|}{S1} & \multicolumn{2}{c|}{S2} & \multicolumn{2}{c|}{S4} \\ \hline
%    %\textbf{HLL initially empty?} & yes & yes & yes & no & yes & no \\ \hline
%      \textbf{Original Card. Est.} & 0 & 0 & 0 & 1,001 & 0 & 1,003 \\ \hline
%       \textbf{\# Targeted buckets $B$} & 1 & 128 (= m/2) & 1 & - & 1 & - \\ \hline
%     \textbf{\# Items added} & 977 & 125,091 & 980 & 125,153 & 1,003 & 204,127 \\ \hline
%    \textbf{Final Card. Est.} & 1 & 177 & 1 & 1,058  & 1 & 1,003 \\ \hline
%    \textbf{\# Resets} & 1,148 & 1,655 & - & - & - & - \\ \hline
%
%\end{tabular}}
%\label{table:tab2bis}
%\end{table*}
...
We set attack parameter $\epsilon$ to $\frac{1}{2}$ and then $\frac{1}{4}$ in the attack on an initially empty HLL sketch. We recall that this will lead to the number of empty buckets $C_0$ to be equal to $\frac{m}{2} (=8,192)$ and $\frac{m}{4} (=4,096)$ respectively.
The results are averaged over 50 iterations and reported in Table~\ref{table:tab3} and Figure~\ref{fig:graphRedis}.
Similarly to the case for classic HLL, our empirical results match closely with our theoretical analysis.

\begin{table*}[tb!]
\begin{center}
\caption{Attack results on initially empty sketches for HLL in Redis (scenario S2), \\ averaged over 50 iterations.}\label{table:tab3}
%\begin{tabular}{| m{11em} | m{4em} | m{4.2em} |}
\begin{tabular}{| l | r | r |}
    \hline
    \textbf{Original Card. Est.} & 0 & 0 \\ \hline
     \textbf{$\epsilon$} &  0.5 & 0.25  \\ \hline
    \textbf{\# Items added} &  524,2104 & 785,920 \\ \hline
    \textbf{Final Card. Est.} & 13,185 & 35,938\\ \hline
\end{tabular}
\end{center}
\end{table*}
%\todo[inline]{Results from Redis or GBQ if we get them.}

%    \textbf{Original Card. Est.} & 0 & 139,967 \\ \hline
%     \textbf{$\epsilon$} &  0.5 & - \\ \hline
%    \textbf{\# Items added} & 524,210 & 524,454\\ \hline
%    \textbf{Final Card. Est.} & 13,185 & 139,967 \\ \hline
...
We have given a formal analysis of using a PRF as a countermeasure to our attacks. The use of a keyed function sacrifices HLL mergeability when the keys in the different HLLs are independently generated. It is an open problem to find methods for securing HLL sketches against adversarial inputs whilst retaining some form of mergeability. This may also require the introduction of entirely new data structures that trade increased storage for mergeability and improved security. It is also an open problem to extend our simulation-based approach to formal analysis to other data structures.
...
The authors thank Dennis Hofheinz and Anupama Unnikrishnan for useful discussions on simulation-based definitions, Tom Shrimpton for suggesting to look at GoogleBigQuery, Eylon Yogev for drawing our attention to related work on streaming algorithms with adversarial inputs, and Angelika Steger for help with balls-into-bins arguments.
\fi
