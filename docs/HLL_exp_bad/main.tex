...
The S2 adversary knows $h$ so it selects items $x$ from ${\mathcal{X}}$ for which the $\ell$ rightmost bits of $h(x)$ start with up to $t-1$ ``0'' bits. In this case, since the relevant bucket $j$ already has $M[j] \ge t$, the bucket's counter is not updated, and the cardinality estimate of the HLL sketch remains unchanged. 
...
We create an HLL sketch using $n=8$ and hence $m=256$ buckets and, for attacks in the setting of an initially non-empty target HLL sketch, initialise it with $t \cdot m$ log $m$ random items representing honest users' data.
...
As a second experiment, and in order to get results that can be compared to the work of~\cite{hllvuln}, we mount our attack in their own setting: instead of picking from ${\mathcal{X}}$, the adversary has to choose what to insert from a subset of 250,000 random items that are given to it. 

Finally, for completeness, we also run the attacks against HLL sketches filled with adversarial inputs only, thus that are initially empty. We vary the value of $B$ in the attack for scenario S1 to demonstrate some of the available trade-offs. For simplicity and to keep the attack as striking as possible, we set $B$ to 1 in the attacks on an initially empty HLL sketch in scenarios S2 and S4. All attacks were run 50 times and we report average outcomes. Our results are reported in Tables~\ref{table:tab1} and~\ref{table:tab2}, which detail the estimated cardinality at the beginning of the attack, the number of items added by the adversary, and the cardinality estimate after adding the adversary's items.

These empirical results confirm our analysis on several points.

First, when attacking a sketch already containing $t \cdot m$ log $m$ random items, we can see that the adversary is on average able to insert a fraction $1-1/2^t$ of the $2^{20}$ items in our starting set in scenario S2, and a fraction $1-H_M^{-1}$ in scenario S4. For example for $t = 1$, the adversary can insert half of the items in S2, and about 83\% in S4. The cardinality estimate is, on average, increased slightly in S2 (from 1,411 to 1,430) and remains at the same starting value (1,416 on average) in S4. In both cases, we inserted more than half a million items whilst increasing the cardinality estimate by about $1\%$!  These results align well with our theoretical analysis.  
...
\begin{figure*}[htp]
    ...
    \begin{minipage}{0.45\textwidth}
        \centering
        \caption{Attack results as function of t in scenario S2 with initially non-empty HLL sketch.}
        \includegraphics[width=0.9\textwidth]{figures/graph_S2.pdf}
    \end{minipage}\hfill
    \begin{minipage}{0.45\textwidth}
        \centering
        \caption{Attack results as function of t in scenario S4 with initially non-empty HLL sketch.}
...
\begin{table*}[tb!]
\begin{center}
\caption{Attack results for HLL in Redis, averaged over 50 iterations.}
