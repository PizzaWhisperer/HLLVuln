...
\usepackage{url}
\usepackage{hyperref}
\title{\huge HyperLogLog, exponentially bad in Adversarial Setting}
...
\begin{abstract}
Computing the count of distinct elements in massive data sets is often necessary but memory or computationally expensive. The HyperLogLog (HLL) algorithm \cite{hll2} estimates a data stream’s cardinality while using significantly less memory at the expense of some accuracy. This trade off makes it a really attractive solution that can find its use in a wide range of applications, such as database management or network monitoring. Its recent popularity made it an interesting target for attackers, and vulnerabilities are starting to be acknowledged. This works puts itself in the continuity of \cite{hllvuln} and improves their attack on the HLL accuracy. We show that disclosing too much information about the HLL internals opens the door to estimate manipulation by dishonest users, that may result in an exponential reduction from the true cardinality.
\end{abstract}
}
...
Obvious approaches to computing the cardinality of a big data set, such as sorting the elements or simply maintaining the set of unique elements seen, are impractical. HyperLogLog is the most widely used cardinality estimator nowadays, and the first reviews of its capabilities show a lack of security both in privacy and correctness. We move forward in exploiting the vulnerabilities of the sketch and show that an attacker can easily manipulate the HLL estimate with very small knowledge about the sketch and a limited number of computations.\\

The rest of the paper is organized as follows. Section 2 gives an overview of the HyperLogLog algorithm. Section 3 presents our attack and evaluates its impact under different adversarial models. Section 4 provides a conclusion and some ideas for future work.
