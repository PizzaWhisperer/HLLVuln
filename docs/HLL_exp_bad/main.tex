\newif\ifacmversion % turn on ACM style (can be short or full)
\acmversiontrue

\newif\ifanon % turn on ACM style (can be short or full)
\anontrue

\ifacmversion

\documentclass[sigconf, anonymous, dvipsnames, draft]{acmart} %dvipsnames for old/new macros
...
\acmConference[Anonymous Submission to ACM CCS 2021]{ACM Conference on Computer and Communications Security}{Due 6 May 2021}{Seoul, South Korea}
\acmYear{2021}
...
%\author{{\rm Kenneth G. Paterson}\\ {\rm ETH Zurich} \and {\rm Mathilde Raynal\thanks{Work done while an exchange student at ETH Zurich.}}\\ {\rm EPFL}}
...
\begin{itemize}
\item Google and counting stuff.\todo{add more detail here -- their blogs, their paper, the use in Google Big Query.} 
...
\todo[inline]{make the model a contribution... ``To achieve this... introduce simulation-based security model for cardinality estimators'', etc}
\todo[inline]{Simplicity of attacks as a strength; only complexity comes in S1 attack which we don't consider that interesting!}
\todo[inline]{No e2e attack on a specific system; rather paper is written in general form since the attacks apply broadly.}
\todo[inline]{Highlight the analysis of OSDI paper~\cite{flexswitch} as an example of the potential of the attacks.}

The rest of the paper is organised as follows. Section~\ref{sec:related} immediately below discusses related work. Section~\ref{sec:overview} gives an overview of HLL. Section~\ref{sec:attacks} presents our different adversarial models. In Section~\ref{sec:manip}, we describe attacks on HLL and evaluate their impacts under the different adversarial models. Section~\ref{sec:redis} considers the security of the more modern version of HLL due to Ertl~\cite{hllnew} that is used in Redis. Our experimental results are discussed and related back to specific use cases in Section~\ref{sec:exp}. Section~\ref{sec:formal} provides our formal security analysis of HLL. Section~\ref{sec:conclusions} provides our conclusions and some ideas for future work.
...
%\todo{update: we will get privacy for free from our sim based security model if sim does not use input $x$.}
...
So to roughly estimate the cardinality of a stream, we need only to store a representation of $k$, the length of the largest observed string of leading zero-bits for items in the stream. This provides a very compact mechanism for estimating a stream's cardinality -- if the true cardinality is $N$, then just $\mathcal{O}(\log\log(N))$ bits of storage are needed to compute the cardinality estimate.

This simple approach to cardinality estimation suffers from large variance in the estimation. In order to improve the variance, HLL uses many estimators in parallel instead of one and averages the results. Each estimate is stored in its own register that we call a bucket. We refer to the collection of buckets and their contents as the HLL \emph{sketch}. To map a value $x$ to one of $m=2^n$ buckets, \cite{loglog} suggests using the first $n$ bits of $h(x)$ as a bucket index, and then computing the longest sequence of leading zero bits on the remaining $\ell$ bits of $h(x)$. The output length of $h(\cdot)$ is thus $n+\ell$, fixed to 32 in the original HLL paper~\cite{hll}. Estimates from all buckets are averaged using the harmonic mean and scaled by a constant $\alpha_m$, where $\alpha_m$ is empirically computed to correct a systematic multiplicative bias.

In the original HLL proposal~\cite{hll}, relatively small and large cardinalities are handled separately. This is to correct for systematic errors. When the raw cardinality estimate is below $\frac{5}{2}m$, the Hit Counting algorithm of Whang \emph{et al.}~\cite{hitcounting} is used to produce the final cardinality estimate. This estimator uses the number of empty buckets (whose register value is 0) instead of the number of leading zeros in the stream. A similar correction is applied for large cardinalities in order to take into account the likelihood of hash collisions. In addition, the cardinality estimate is usually rounded to a whole number before being output. The original HLL algorithm from~\cite{hll} is shown in Fig.~\ref{fig:hll}. This algorithm is able to estimate cardinalities greater than $10^9$ with a typical standard error of 2\%, using only 1.5 kB of memory~\cite{hll}.
...
\vspace{0.5em}
\textbf{Algorithm} HyperLogLog (\textbf{input} ${\mathcal{X}}$ : multiset of items from domain $\mathcal{D}$).\\
...
For simplicity, we take ${\mathcal{X}}$ to be the set of 4-character long ASCII strings, with $\lvert {\mathcal{X}} \rvert = 7,311,616= 2^{22.8}$. We stress that this choice of inputs is merely illustrative and the results do not in any way depend on choosing ASCII strings. We choose $h$ to be the 32-bit MurmurHash3 function from~\cite{murmur3code}.
...
\todo[inline]{This ends up being pretty harsh on this NSDI paper, but hey.}

\section{Formal Security Analysis}\label{sec:formal}

In this section we present a formal security analysis of HLL, focussing on how to augment the original HLL design with cryptographic mechanisms and thereby provably prevent attacks.
...
\subsection{Intuition for Formal Security Analysis for S2, S3, S4}
...
We first begin by explaining the high-level idea of our approach. We allow an adversary ${\mathcal{A}}$ to interact with an HLL via various oracles that capture its capabilities in each of the three scenarios S2, S3 and S4: it may have an \texttt{insert($\cdot$)} oracle allowing it to add values to the HLL, a \texttt{read} oracle that returns the HLL cardinality estimate, and a \texttt{corrupt} that returns all the HLL bucket contents (but not any secret key $K$); in S4 it has all three oracles. In addition, ${\mathcal{A}}$ always has access to an \texttt{init($\cdot$)} oracle by which it receives the HLL parameters $n$, $\ell$, $\alpha_n$ needed to initialise the HLL sketch.

In the security model, ${\mathcal{A}}$ interacts in one of two worlds. In the real world, ${\mathcal{A}}$ interacts with a real HLL, operating as in Figure~\ref{fig:hll} but with the hash function $h$ replaced by a keyed function $F: \{0,1\}^k \times {{\mathcal{D}}} \rightarrow  \{0,1\}^{n+\ell}$ whose key is not available to the adversary. In the ideal world, ${\mathcal{A}}$ interacts with a simulator ${\mathcal{S}}$ which ignores the input for ${\mathcal{A}}$'s \texttt{insert($\cdot$)} queries but nevertheless provides a ``good'' simulation of the oracle responses to ${\mathcal{A}}$. Importantly, ${\mathcal{S}}$ will handle all of ${\mathcal{A}}$'s requests to insert an item $x$ using \emph{random sampling}, that is, by setting $y \leftarrow_{\$} \{0,1\}^{n+\ell}$ instead of $y: = F_K(x)$. 

We will show that when $h$ is replaced by the keyed function $F$	 then any adversary ${\mathcal{A}}$ that can distinguish between the real and ideal worlds can be used to build an adversary ${{\mathcal{B}}}$ which breaks the PRF security of $F$. Under the assumption that $F$ is a good PRF, no such efficient adversary ${{\mathcal{B}}}$ can exist, and we may conclude that (up to PRF security) ${\mathcal{A}}$ cannot tell that it is operating in the ideal world instead of the real world. 

But in the ideal world, ${\mathcal{A}}$ interacts with a simulator that uses random sampling for the $y$ values. This effectively reduces ${\mathcal{A}}$ to operating in the setting where the average case analysis of HLL applies, as per~\cite{hll}. Here, ${\mathcal{A}}$ has no advantage when it comes to influencing the HLL cardinality estimate. It follows that HLL, when instantiated with PRF $F$, is secure against attacks in S4 (and therefore in the weaker attacks models S2 and S3 as well). 
%Recall also that the simulator ${\mathcal{S}}$ ignores the values $x$ being inserted by ${\mathcal{A}}$. This enables us to get certain confidentiality properties for free when HLL is implemented using a PRF in place of $h$. For example, we can argue that the bucket contents (obtained via a \texttt{corrupt} query) reveal nothing to ${\mathcal{A}}$ about what values $x$ were inserted into the HLL.

We will conclude our analysis by discussing how $F$ can be instantiated.

\subsection{Formal Security Model for HLL}

We consider adversary ${\mathcal{A}}$ to be an algorithm running in a security game. The game begins with the selection of a bit $b$ uniformly at random. Adversary ${\mathcal{A}}$ interacts with up to four oracles which are handled by the game; different combinations of oracles define scenarios S2, S3, S4. After making some queries, ${\mathcal{A}}$ returns a bit $b'$ estimating whether it is interacting with a real HLL instantiation ($b'=0$) or an ideal one ($b'=1$). For $i=1, 2, 3$, we define ${\mathcal{A}}$'s S$i$-Real-or-Ideal advantage against HLL to be:
\[
\mbox{Adv}^{\mbox{\tiny S$i$-RoI}}_{\mbox{\tiny HLL}}({\mathcal{A}}) := \lvert 2\Pr[b' = b] - 1 \rvert \, .
\]
...
\begin{itemize}
\item \texttt{init($\cdot$)}: this oracle receives as input the HLL parameters ($n$, $\ell$, $\alpha_n$) and performs initialisation of the HLL sketch. It also selects a key $K \leftarrow_{\$} \{0,1\}^k$ for keyed function $F: \{0,1\}^k \times {{\mathcal{D}}} \rightarrow  \{0,1\}^{n+\ell}$. This oracle is available in all of S2, S3, S4, but can only be called once as ${\mathcal{A}}$'s first oracle query.
\item \texttt{insert($\cdot$)}: on input $x \in {{\mathcal{D}}}$, the oracle sets $y = F_K(x)$ and follows the procedure in Figure~\ref{fig:hll} to insert value $x$ into the HLL sketch (that is, it uses the first $m$ bits of $y$ to determine a bucket index $j$ and the next $\ell$ bits of $y$ to determine a bit-string $w$; then $\rho(w)$ is used to update the bucket at index $j$ via $M[j]:= \max(M[j],\rho(w))$). This oracle is available in all of S2, S3, S4. Without loss of generality (since HLL and variants of it we consider here have a deterministic insertion procedure), we assume all queries $x$ made by ${\mathcal{A}}$ to \texttt{insert($\cdot$)} are distinct. We let $q$ denote the number of insertion queries made by ${\mathcal{A}}$ during the course of its attack.
\item \texttt{read}: this oracle returns the HLL cardinality estimate as per Figure~\ref{fig:hll}. We allow multiple queries to this oracle and make it available to ${\mathcal{A}}$ in S3 and S4, but not S2.
...
In many simulation-based security proofs, the simulator ${\mathcal{S}}$ depends on ${\mathcal{A}}$, and security is quantified via a statement of the form ``For all adversaries ${\mathcal{A}}$, there exists a simulator ${\mathcal{S}}$ such that $\ldots$'' with ${\mathcal{S}}$ being described in the proof of a specific scheme. In our analysis we are able to provide a universal simulator, enabling us to make a stronger statement of the form ``There exists a simulator ${\mathcal{S}}$ such that, for all adversaries ${\mathcal{A}}\ldots$''. We describe this simulator now, in terms of how it handles  ${\mathcal{A}}$'s oracle queries in the ideal world:
...
Let simulator $\mathcal{S}$ be as described above. Suppose ${\mathcal{A}}$ is any S4 adversary against HLL instantiated with a keyed function $F: \{0,1\}^k \times {{\mathcal{D}}} \rightarrow  \{0,1\}^{n+\ell}$. Suppose ${\mathcal{A}}$ either plays in the real world or in the ideal world with $\mathcal{S}$. Then we can construct from ${\mathcal{A}}$ a PRF adversary ${{\mathcal{B}}}$ against $F$ such that:
\[
 \mbox{Adv}^{\mbox{\tiny PRF}}_{F}({{\mathcal{B}}}) = \mbox{Adv}^{\mbox{\tiny S4-RoI}}_{\mbox{\tiny HLL}}({\mathcal{A}}) \,.
\]
Moreover, if ${\mathcal{A}}$ makes $q$ queries to its \texttt{insert($\cdot$)} oracle, then ${{\mathcal{B}}}$ makes $q$ queries to its real or random oracle. Finally, the running time of ${{\mathcal{B}}}$ is (roughly) the same as that of ${\mathcal{A}}$.
\end{theorem}

%Informally, if $F$ is a good PRF, then $\mbox{Adv}^{\mbox{\tiny PRF}}_{F}({{\mathcal{B}}})$ should be small for any adversary ${{\mathcal{B}}}$ running in a reasonable amount of time; hence the advantage of any S4 adversary  ${\mathcal{A}}$ against HLL using PRF $F$ is also small. Note that our proof still applies for the improved algorithm of~\cite{hllnew} that is used in Redis. The only changes to the proof are to use the Redis approach for computing responses to \texttt{read} queries in place of the classical HLL approach.

\begin{proof}
PRF adversary ${{\mathcal{B}}}$ against $F$ is built as follows. Let $d$ denote the hidden bit determining whether ${{\mathcal{B}}}$ receives real responses of the form $F_K(x)$ where $K$ is a uniformly random key ($d=0$) or responses from a truly random function ($d=1$); ${{\mathcal{B}}}$ runs S4 adversary ${\mathcal{A}}$ and responds to each of its queries as follows:
\begin{itemize}
\item \texttt{init}: here ${{\mathcal{B}}}$ initialises an HLL sketch in accordance with Figure~\ref{fig:hll}. Thus it initialises an array $M[j]$ in which each entry is set to 0. 
\item \texttt{insert($\cdot$)}: on input $x \in {{\mathcal{D}}}$,  ${{\mathcal{B}}}$ makes a query on $x$ to its real-or-random oracle and receives value $y$; ${{\mathcal{B}}}$ uses $y$ to update the HLL bucket $j$ generated via $j=1 + <y_1y_2...y_{n}>_2$ as in the regular HLL algorithm (shown in Figure~\ref{fig:hll}).
\item \texttt{read}: ${{\mathcal{B}}}$ uses its knowledge of the buckets' contents to compute and return the HLL cardinality estimate in accordance with Figure~\ref{fig:hll}. 
\item \texttt{corrupt}: ${{\mathcal{B}}}$ returns the vector $(M[j])_{j=1}^{m}$ of HLL bucket contents. 
\end{itemize}
When ${\mathcal{A}}$ outputs a bit $b'$, ${{\mathcal{B}}}$ outputs its own bit $d'=b'$. Note that when $d=0$, ${{\mathcal{B}}}$ plays against the real $F$ with a random key $K$ and provides a simulation for ${\mathcal{A}}$ that perfectly instantiates the real world. On the other hand, when $d=1$, ${{\mathcal{B}}}$ plays against a truly random function and provides a simulation for ${\mathcal{A}}$ that perfectly instantiates the ideal world with simulator $\mathcal{S}$ (to see this, note that sampling the outputs $y$ of a truly random function at distinct inputs $x$ is identical to simply sampling $y$ uniformly at random). The equality of advantages in the statement of the theorem now follows immediately. The remainder of the proof follows on counting queries and noting that the HLL algorithm for computing cardinality is efficient, so ${{\mathcal{B}}}$'s running time due to handling \texttt{read} queries is not increased significantly compared to that of ${\mathcal{A}}$.
\end{proof}

We reiterate our central point: the theorem shows that no efficient S4 adversary ${\mathcal{A}}$ can tell that it is playing against our simulator ${\mathcal{S}}$ using truly random values $y$ instead of with values obtained from a PRF, except with a small advantage. This means that ${\mathcal{A}}$ is effectively reduced to attacking HLL in the exact same setting used in the average case analysis of HLL from~\cite{hll}, wherein it is proved that the HLL cardinality estimate endures relative errors of the form $1.04/\sqrt{m}$. From this we can conclude that no efficient S4 adversary ${\mathcal{A}}$ can manipulate the HLL cardinality estimate to its benefit.

Note that our security analysis using a simulation-based definition does not require us to specify a winning condition for the adversary. Rather, we infer from inspecting the simulator what the adversary can (or cannot) achieve in the ideal world, and show that no efficient adversary can distinguish the real and ideal worlds. This stands in contrast to the game-based approach of~\cite{CCS:ClaPatShrPS19} which needs a complex definition to say when the adversary has successfully manipulated a probabilistic data structure. This reveals a strength of the simulation-based approach in more complex settings like ours.

\subsection{On Confidentiality}

An HLL can leak information about values $x$ that were previously inserted. Consider the following attack from~\cite{cardestprivacy}: to test whether a value $x$ was previously inserted into the HLL by an honest user (say), the adversary makes the sequence of queries: \texttt{read}, \texttt{insert($x$)}, \texttt{read}. If the two cardinality estimates are different, the adversary can conclude definitively that $x$ was \emph{not} previously inserted into the HLL; if they are the same, it can estimate (with some error) that $x$ was previously inserted.\footnote{Errors can arise in two ways when $x$ was not previously inserted: either inserting $x$ changes a bucket value, but the change is masked by rounding when the cardinality estimate is computed, or inserting $x$ does lead to a change in a bucket value because $\rho(w)$ computed from $x$ is too small.} This attack still works even if HLL is implemented using a PRF and is unavoidable in view of the oracle queries (and in real settings, the API) available to the adversary. So such privacy leaks are an inherent feature of HLL if a suitably rich API is provided. Note that this attack does not contradict our main result, which concerns prevention of cardinality manipulation rather than confidentiality. 

Still, simulation-based security notions are often used to model confidentiality properties, so it is tempting to think that our security definition might say something about confidentiality of the HLL inputs $x$. Our existing model does not, since ${\mathcal{A}}$ chooses all the inputs $x$ and so ``knows'' them already. However, it is easily modified to provide a confidentiality notion. For example, we can consider the insertion of honestly generated values $x$ that are drawn from some probability distribution $p$ on $\mathcal{D}$ in a first phase, with ${\mathcal{A}}$ then having access to only \texttt{read} and \texttt{corrupt} oracles in a second phase (we have to remove the \texttt{insert($\cdot$)} oracle in view of the general attack from~\cite{cardestprivacy}). 
...
We leave the further exploration of how to adapt our simulation-based security definitions to capture confidentiality notions to future work. 

\subsection{Is a PRF Required?}

It is natural to wonder whether a PRF is really required in place of hash function $h$ in order to make HLL secure, or whether a weaker cryptographic primitive would suffice. For example, perhaps using a keyed universal hash function (UHF) $h_K(\cdot)$ would be sufficient.

In general, UHFs do not preserve confidentiality of their keys: the key may be recovered from a small number of UHF outputs. Consider, for example, the UHF built from polynomial hashing over a finite field $\mathcal{F}$. Here, inputs $x$ are encoded as polynomials  $p_x \in \mathcal{F}(X)$, the key is a uniformly random field element, and the UHF output $h_K(x)$  is equal to $p_x(K)$, obtained by polynomial evaluation at $K$. It is well-known that an adversary who knows the input $x$ can determine the key $K$ by finding roots of polynomials over $\mathcal{F}$, using Berlekamp's algorithm~\cite{Berlekamp}. If the adversary can choose $x$, then it can make the polynomials $p_x$ have low degree, including of degree 1, making the attack easier to perform. 

Now consider using such a UHF in place of $h$ in attack scenario S4. For concreteness, we assume $\mathcal{F}$ equals $\textsf{GF}(2^{128})$, so our UHF has 128-bit keys and 128-bit outputs. The output is then truncated to provide the 32 bits needed by HLL (or, say, the 64 bits consumed by the Redis variant of HLL). In S4 an adversary $\mathcal{A}$ can perform a series of pairs of \texttt{insert($x_i$)} and \texttt{corrupt} queries on chosen values $x_1, x_2, \ldots$. With high probability, each  \texttt{insert} query touches a fresh bucket in the HLL sketch. Then each \texttt{corrupt} query reveals to $\mathcal{A}$ which bucket was touched by the preceding \texttt{insert} query. In turn, this leaks partial information about the corresponding UHF output $p_{x_i}(K)$. 

Going further, the S4 adversary has complete control over the values $x_i$, so it can choose them so that the polynomials $p_{x_i}$ are of degree 1 over $\textsf{GF}(2^{128})$. Then $K$ can be determined by writing everything bit-wise and solving a linear system. The idea is that, when $x_i$ has 128 bits, we can write $h_K(x_i) = p_{x_i}(K) = x_i \cdot K$ where the multiplication is over $\textsf{GF}(2^{128})$ (and we turn bit-strings into field elements in a standard way). Then each bit of $h_K(x_i)$ is a linear combination of the bits of $K$, with the coefficients being determined by the known bits of $x_i$ and the degree 128 irreducible polynomial $f$ over $\textsf{GF}(2)$ that determines how field multiplication works at the bit level. 
 
The above attack exploits specific properties of the chosen UHF, but demonstrates that UHFs are not a suitable tool for protecting HLL in general. It is still possible that UHFs could be safely used in weaker attack scenarios, e.g.\ S2. We leave a detailed study to future work. 
