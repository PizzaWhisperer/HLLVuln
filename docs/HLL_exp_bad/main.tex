...
\begin{minipage}{40em}
\begin{flushleft}
\textit{Let $h : \mathcal{Y} \rightarrow \{0, 1\}^{32}$ hash data from some domain $\mathcal{Y}$ to binary $32$-bit strings.\\
...
\vspace{0.5em}
\textbf{Algorithm} HyperLogLog (\textbf{input} ${\mathcal{X}}$ : multiset of items from domain $\mathcal{Y}$).\\
...
In the security model, the game selects at random one of two worlds by sampling a bit $b$. In the real world ($b=0$), ${\mathcal{A}}$ interacts with the scheme HLL-$F$. This is the same as HLL operating as in Figure~\ref{fig:hll} but where the hash function $h$ is replaced by a keyed function $F: \{0,1\}^k \times {{\mathcal{Y}}} \rightarrow  \{0,1\}^{n+\ell}$ whose key $K$ is sampled uniformly at random at the beginning of the game and not available to the adversary; ${\mathcal{A}}$ has access to the relevant selection of oracles and completes its execution with some output $\textsf{out}_{\mathcal{A}}$ (a bit-string).
...
We now formalise the above intuition. Our definitions are specific to HLL-$F$ as the object of analysis, but are easily generalised to any cardinality estimator with the same syntax as HLL-$F$ (formally defined by a triple of algorithms (\texttt{Init}, \texttt{Insert}, \texttt{Read})).
...
\begin{itemize}
\item \texttt{init($\cdot$)}: this oracle receives as input the HLL-$F$ parameters ($n$, $\ell$, $\alpha_m$) and performs initialisation of the HLL-$F$ sketch, including generating the array of $m=2^n$ buckets $M[j]$ and initialising all the entries to $0$, and selecting a key $K \leftarrow_{\$} \{0,1\}^k$ for keyed function $F: \{0,1\}^k \times {{\mathcal{Y}}} \rightarrow  \{0,1\}^{n+\ell}$. This oracle has no output. It is available in all of S2, S3, S4. We assume it is only called once as ${\mathcal{A}}$'s first oracle query.
\item \texttt{insert($\cdot$)}: on input $x \in {{\mathcal{Y}}}$, the oracle implements HLL-$F$ by setting $y = F_K(x)$ and following the procedure in Figure~\ref{fig:hll} to insert $x$ into the HLL sketch (that is, it uses the first $n$ bits of $y$ to determine a bucket index $j$ and the next $\ell$ bits of $y$ to determine a bit-string $w$; then $\rho(w)$ is used to update the bucket at index $j$ via $M[j]:= \max(M[j],\rho(w))$). This oracle has no output. It is available in all of S2, S3, S4. Since HLL and variants of it we consider here have the property that reinserting an item $x$ does not change the internal state of the HLL sketch (nor the cardinality estimate computed from it) we assume without loss of generality that all queries $x$ made by ${\mathcal{A}}$ to \texttt{insert($\cdot$)} are distinct.
...
\item \texttt{insert($\cdot$)}: on input $x \in {{\mathcal{Y}}}$,  ${\mathcal{S}}$ ignores $x$, sets $y \leftarrow_{\$} \{0,1\}^{n+\ell}$, and proceeds to update the buckets using this value of $y$ as in the original HLL algorithm in Figure~\ref{fig:hll}.
...
Note also that our definition is given with respect to a \emph{specific} simulator, but could be generalised to allow any simulator that performs random sampling when handling \texttt{insert($\cdot$)} queries.

Before stating our main result, we require one further standard definition:
\begin{definition}
Let $F: \{0,1\}^k \times {{\mathcal{Y}}} \rightarrow  \{0,1\}^{n+\ell}$.
...
Our HLL construction requires a PRF $F: \{0,1\}^k \times {{\mathcal{Y}}} \rightarrow  \{0,1\}^{n+\ell}$. Here, the set ${{\mathcal{Y}}}$ represents the space of possible inputs to HLL which can be modelled by a set of bit-strings of some maximum length. This means that $F$ needs to be Variable Input Length PRF (VIL-PRF). 
