...
Suppose ${\mathcal{A}}$ is an S4 adversary against HLL instantiated with a keyed function $F: \{0,1\}^k \times {{\mathcal{D}}} \rightarrow  \{0,1\}^{n+\ell}$. Then we can construct from ${\mathcal{A}}$ a PRF adversary ${{\mathcal{B}}}$ against $F$ such that:
\[
...
We also recall that our ${\mathcal{S}}$ ignores ${\mathcal{A}}$'s inputs $x$ in handling \texttt{insert($\cdot$)} queries. In traditional simulation-based security notions, this would imply that the ``leakage'' given to the simulator does not include the input values $x$, and one would then conclude by analogy to, say, semantic security for encryption that the HLL implemented using a PRF offers confidentiality for the inputs $x$. This is not quite correct here, since ${\mathcal{A}}$ chooses all the inputs $x$ and so ``knows'' them already. To address confidentiality, one would need instead to consider $x$ that are drawn from some distribution on $\mathcal{D}$ or consider two-stage adversaries that ``forget'' the $x$ values chosen in its first stage by not passing state to its second stage. 

Even here, however, an HLL can leak information about values $x$ that were previously inserted. Consider the following attack from~\cite{cardestprivacy}: to test whether a value $x$ was previously inserted into the HLL, make the sequence of queries: \texttt{read}, \texttt{insert($x$)}, \texttt{read}. If the two cardinality estimates are different, one can conclude definitively that $x$ was \emph{not} previously inserted into the HLL; if they are the same, one can estimate (with some error) that $x$ was previously inserted. This attack still works even if HLL is implemented using a PRF and is unavoidable in view of the oracle queries (and in real settings, the API) available to the adversary. So such privacy leaks are an inherent feature of HLL if a suitably rich API is provided. Note that this attack does not contradict our main result, which concerns prevention of cardinality manipulation rather than confidentiality. 

More positively, what we can say is that our simulation-based security definition does imply confidentiality of inputs in the  case of a model where an honest party makes a sequence of insertions and where ${\mathcal{A}}$ is then limited to \texttt{read} and \texttt{corrupt} queries. \todo[inline]{explain why.}
...
%It does not matter that ${\mathcal{A}}$ can adaptively choose its inputs $x$ in \texttt{insert($\cdot$)} queries, nor that it can access \texttt{read} and \texttt{corrupt} queries. For each \texttt{insert($\cdot$)} query, a fresh value $x$ leads to a new random $y$ being generated and used to update a bucket, as in the average case analysis of HLL. The \texttt{read} and \texttt{corrupt} queries do leak information about $F$'s outputs to the adversary, but such outputs are indistinguishable from random under the assumption that $F$ is a PRF. 

\todo{explain why a sim-based approach is better, more general, does not require a specific winning condition.}

\todo{Why UHFs don't work in general so why we need a PRF -- key recovery is possible in S4; partial info about processed values and therefore keys  leaks from Card Est in S2, though harder to make an attack from this. }
