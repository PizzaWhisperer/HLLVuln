...
%\kenny{TODO: clarify more what this is terms of a ''test oracle''; its stronger than described because the adversary can test the effect of an insertion without making the insertion!s}
...
These empirical results confirm our impact analysis on several points. First, when attacking a sketch already containing some items, we can see that the adversary is able to hide approximately half of the items in our starting set under S2, and around 83\% under S4. %( $205$'$000 \approx 250'000 \times 0.82 \approx 250$'$000\times(1-\frac{1}{2\times H_c})$, with $H_c\approx\frac{1'000}{256}=3.9$)
...
To assess the efficiency of our attacks, we can compare our results from Table~\ref{table:tab2} with the attack presented in Section 5.1 of~\cite{hllvuln}. In that paper, the authors target the Redis~\cite{redis} open-source implementation of HLL and perform the attack against an empty sketch. Given a set of 250,000 items, the adversary could choose ``74,390 distinct items and obtain an HLL estimate of only 15,780``~\cite{hllvuln}, achieving a five-fold reduction from the true cardinality. The authors present this setup as the M2 scenario, without providing any explanation. When looking at the open-source code, we could gain details on the implementation that any attacker could obtain, and thus argue that based on this knowledge, the adversarial scenario is actually M1 (S2 in this work), making the comparison relevant. In comparison, the adversary following our strategy is able to add 125,000 distinct items while keeping the cardinality estimate at 367, providing a reduction by a factor of 340 instead of 5.
