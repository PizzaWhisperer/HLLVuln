...
Here the attack is somewhat more complex. We begin by explaining the core idea, then a simplistic version of our attack, then a more complex but performant version, and finally, we give a heuristic performance analysis of the attack.
...
We focus here on scenarios S2 and S4, and only sketch an attack below for S1.
...
As a proof-of-concept, we implemented our attacks against the HLL implementation from~\cite{clahll}, since it is faithful to the original description of HLL given in~\cite{hll}.\footnote{Our attack code is available at \href{https://github.com/PizzaWhisperer/HLLVuln}{\url{https://github.com/PizzaWhisperer/HLLVuln}}.}. For simplicity, our items are 4-character long ASCII strings, giving us a set ${\cal X}$ of $7,311,616= 2^{22.8}$ distinct items that can be inserted (hence we have $t=22.8$ in our attacks). We choose $h$ to be the 32-bit Murmur3 hash function from~\cite{murmur3code} (note that the library allows the hash function to be specified; we pick Murmur3 for our attack and consider it reasonable that the adversary should know the hash function in use).
...
Secondly, when attacking an empty HLL sketch, we can see that in all scenarios, the final cardinality estimate is always exactly $m \cdot \ln (m/(m-B))$ after inserting a fraction $\frac{B}{m}$ of the items, as predicted by our analysis.

We can also compare our results from Table~\ref{table:tab2} with results from the attack presented in Section 5.1 of~\cite{hllvuln}. In that paper, the authors target the Redis~\cite{redis} open-source implementation of HLL and perform the attack against an empty sketch filled with adversarial inputs only. Given a set of 250,000 items, the adversary could choose ``74,390 distinct items and obtain an HLL estimate of only 15,780''~\cite{hllvuln}, achieving a five-fold reduction from the true cardinality after 4 iterations (resets). The S1 adversary following our strategy is able to add 977 and 97,433 distinct items while holding the cardinality estimate at 1 and 126, respectively. Compared to~\cite{hllvuln}, we can reduce the cardinality estimate by a factor of almost 1,000 instead of 5. This comes at the cost of performing more resets compared to~\cite{hllvuln}.

\todo{Check why in the first table in S1 we end up with a card.\ est.\ of 126 instead of $B=100$. The S1 attack description says we should stay at $B=100$ so something is off!}
...
Left to future work is the task of extending our attacks to HLL variants such as sliding HLL~\cite{slidinghll}, HLL++~\cite{hllpratice} and the version of HLL used in Redis, which relies on the maximum likelihood approach from~\cite{newhll} to perform corrections on cardinality estimates instead of the Hit Counting algorithm.

It would be interesting to find and analyse a strategy for more cleverly ordering the candidates $x$ in our attack in scenario S1 to reduce the total insertion cost of the attack, whilst also keeping the number of resets low.

A formal analysis of using salting or secretly keyed hash functions as a countermeasure to our attacks remains open. This could be done by extending the framework of~\cite{CCS:ClaPatShrPS19}. An interesting cryptographic challenge here is to  improve the security of HLL sketches against adversarial inputs whilst retaining some form of the uncoordinated mergeability that is enjoyed by the original HLL algorithm.
