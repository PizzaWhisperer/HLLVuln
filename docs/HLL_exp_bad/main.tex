...
This completes our analysis of Redis. The key takeaway is that an adversary can expect to insert a constant fraction of ${\cal X}$ into a non-empty Redis HLL filter whilst not increasing its cardinality estimate at all (and whilst keeping the estimated cardinality linear in $m$ in the case where the filter is initially empty).
