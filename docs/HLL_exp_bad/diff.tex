...
%DIF ADD main.tex          Tue Feb  8 15:00:58 2022
...
\DIFdel{= \frac{\alpha_\infty m^2}{m\sigma(C_0/m) + \sum_{k=1}^{\ell}C_k 2^{-k} + m \tau(1-C_{\ell+1}/m)2^{-\ell}} \, .
}\DIFdelend \DIFaddbegin \resizebox{.9\hsize}{!}{$
...
Similarly to the case for classic HLL, our empirical results match \DIFaddbegin \DIFadd{closely }\DIFaddend with our theoretical analysis.

\begin{table*}[tb!]
\DIFdelbeginFL %DIFDELCMD < \small\centering
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{center}
\DIFaddendFL \caption{Attack results for HLL in Redis, \DIFaddbeginFL \DIFaddFL{scenario S2, }\DIFaddendFL averaged over \DIFdelbeginFL \DIFdelFL{30 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{50 }\DIFaddendFL iterations.}\DIFdelbeginFL %DIFDELCMD < \medskip
%DIFDELCMD <      %%%
\DIFdelFL{\makebox[\linewidth]{
...
\DIFaddbeginFL \begin{tabular}{| c | r | r |}
    \hline
...
\section{ \DIFadd{Further analysis of Flexswitches}\DIFaddend }\label{sec:flexswitch} 
...
\section{ \DIFadd{Proof of Theorem 8.1}\DIFaddend}\label{sec:proof} %ridiculous, but had to hard-code this reference
...
\section{ \DIFadd{Breaking HLL-UHF}\DIFaddend}\label{sec:UHF}
