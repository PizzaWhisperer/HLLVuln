...
%DIF ADD main.tex          Sat Feb  5 19:24:19 2022
...
We create an HLL sketch using $n=8$ and hence $m=256$ buckets and, for attacks in the setting of an initially non-empty target HLL sketch, initialise it with \DIFdelbegin \DIFdel{1,000 }\DIFdelend \DIFaddbegin \DIFadd{$t \cdot m$ log $m$ }\DIFaddend random items representing honest users' data.
...
First, when attacking a sketch already containing \DIFdelbegin \DIFdel{1000 }\DIFdelend \DIFaddbegin \DIFadd{$t \cdot m$ log $m$ }\DIFaddend random items, we can see that the adversary is on average able to insert \DIFdelbegin \DIFdel{about half }\DIFdelend \DIFaddbegin \DIFadd{a fraction $1-1/2^t$ }\DIFaddend of the $2^{20}$ items in our starting set in scenario S2, and \DIFaddbegin \DIFadd{a fraction $1-H_M^{-1}$ in scenario S4. For example for $t = 1$, the adversary can insert half of the items in S2, and }\DIFaddend about 83\% in \DIFdelbegin \DIFdel{scenario }\DIFdelend S4. The cardinality estimate is, on average, increased slightly in S2 (from 1,\DIFdelbegin \DIFdel{000 }\DIFdelend \DIFaddbegin \DIFadd{411 }\DIFaddend to 1,\DIFdelbegin \DIFdel{011}\DIFdelend \DIFaddbegin \DIFadd{430}\DIFaddend ) and remains at the same starting value (1,\DIFdelbegin \DIFdel{005 }\DIFdelend \DIFaddbegin \DIFadd{416 }\DIFaddend on average) in S4. In both cases, we inserted more than half a million items whilst increasing the cardinality estimate by about \DIFdelbegin \DIFdel{$0.1\%$}\DIFdelend \DIFaddbegin \DIFadd{$1\%$}\DIFaddend !  These results align well with our theoretical analysis.  
