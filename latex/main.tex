...
We assume that at least $2^{t-1} m\ln m$ items have already been honestly inserted into the target HLL, for some integer $t$ with $1 \leq t \leq \ell$ which will be a parameter of our attack. We assume the S2 adversary somehow knows that this condition holds (e.g.\ through knowledge of when the HLL was initialised and the average rate at which items are inserted into it). An S2 adversary who cannot estimate the number of items in the sketch can always conservatively set $t=1$ and minimise the risk that the required condition does not hold. Notice also that an S3 adversary can use its ability to get cardinality estimates to obtain an estimate for $t$; this is one situation where an S3 adversary can make good use of the extra information available to it as compared to an S2 adversary. In S4, the adversary can use its access to a snapshot to mount a more powerful attack; see Section~\ref{sec:nonemptyHLLS4}. 
...
The results are averaged over 50 iterations and reported in Table~\ref{table:tab3}. Meanwhile, Figure~\ref{fig:graphRedis} shows our results for an initially non-empty sketch as a function of $t$, where $2^{t-1}m\ln m$ items are honestly inserted into the sketch. Similarly to the case for classic HLL, our empirical results in  Table~\ref{table:tab3} and Figure~\ref{fig:graphRedis} match closely with our theoretical analyses.
